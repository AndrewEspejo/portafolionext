export default {
  nav: {
    logo: 'DEV_MIRROR',
    links: [
      { text: 'Work', to: 'mywork' },
      { text: 'Contact', to: 'mycontact' },
    ],
  },
  header: {
    img: 'https://gamecommercefu.s3-us-west-2.amazonaws.com/static/profile.jpg',
    im2:
      'https://gamecommercefu.s3-us-west-2.amazonaws.com/static/83945791_2786732874682801_8152195772914335744_o.jpg',
    imgPlaceholder:
      'data:image/jpeg;base64,',
    text: [
      'Hi!',
      "I'm Jefferson Espejo. A frontend developer apassionate about technologies like React, NodeJS, TailwindCSS, Vue.js and Docker.",
      'I am ',
    ],
    typical: ['frontend developer. 🖥', 2000, 'creative 📱', 2000],
    btnText: 'Discover More',
    placeholder:
      'https://gamecommercefu.s3-us-west-2.amazonaws.com/static/logo512.png',
  },

  stack: {
    title: 'Stack',
    tech: [
      {
        img:
          'https://gamecommercefu.s3-us-west-2.amazonaws.com/static/mongo.png',
        alt: 'mongodb',
      },
      {
        img:
          'https://gamecommercefu.s3-us-west-2.amazonaws.com/static/logo512.png',
        alt: 'react',
      },
      {
        img:
          'https://gamecommercefu.s3-us-west-2.amazonaws.com/static/express.png',
        alt: 'express',
      },
      {
        img:
          'https://gamecommercefu.s3-us-west-2.amazonaws.com/static/node.png',
        alt: 'node',
      },
    ],
    desc: `I use the Stack MERN, it has been provided to me for the development of applications and it is what I am most passionate about so far. Similarly, I like to use tools for CSS such as Tailwind and SASS.`,
  },
};
